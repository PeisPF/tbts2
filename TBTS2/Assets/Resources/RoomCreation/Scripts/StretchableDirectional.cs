...
public class StretchableDirectional : Stretchable
{
    ...
    public override void Stretch(int stretchX, int stretchZ)
    {
        ...
        {
            GameObject first = gameObjectFactory.Duplicate(stretchablePart).Parent(this.transform.parent.transform).Position(stretchDirection * i+(this.transform.position*moveAmount)).Create();
            GameObject second = gameObjectFactory.Duplicate(stretchablePart).Parent(this.transform.parent.transform).Position(stretchDirection * -i+ (this.transform.position * moveAmount)).Create();
